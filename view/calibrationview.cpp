...
void cCalibrationView::OnRender(const float deltaSeconds)
{
	...
	if (g_root.m_baslerCam.m_sensors.size() >= 1)
	{
		...
		const bool update1 = ImGui::DragFloat3("Position Offset1", (float*)&sensor1->m_buffer.m_offset.pos, 0.01f);
		static float rotateY = 0;
		const bool update2 = ImGui::DragFloat("RoateY1", &rotateY, 0.01f);
		const bool update3 = ImGui::DragFloat4("SubPlane1", (float*)&g_root.m_planeSub[0], 0.01f, -1000, 1000, "%.5f");
		if (update3)
		{
			g_root.m_planeSub[0].N.Normalize();
			sensor1->m_buffer.m_planeSub = g_root.m_planeSub[0];
		}
		...
		if (update1 || update2 || update3)
		{
			...
			sensor1->m_buffer.UpdatePointCloudBySelf(g_root.m_3dView->GetRenderer());
		}
	}
	if (g_root.m_baslerCam.m_sensors.size() >= 2)
	{
		cSensor *sensor2 = g_root.m_baslerCam.m_sensors[1];
		const bool update1 = ImGui::DragFloat3("Position Offset2", (float*)&sensor2->m_buffer.m_offset.pos, 0.01f);
		static float rotateY = 0;
		const bool update2 = ImGui::DragFloat("RoateY2", &rotateY, 0.01f);
		const bool update3 = ImGui::DragFloat4("SubPlane2", (float*)&g_root.m_planeSub[1], 0.01f, -1000, 1000, "%.5f");
		if (update3)
		{
			g_root.m_planeSub[1].N.Normalize();
			sensor2->m_buffer.m_planeSub = g_root.m_planeSub[1];
		}
		...
		if (update1 || update2 || update3)
		{
			...
			sensor2->m_buffer.UpdatePointCloudBySelf(g_root.m_3dView->GetRenderer());
		}
	}
