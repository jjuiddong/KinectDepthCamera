...
bool SavePointCloud(const BufferParts& parts, const char* fileName)
{
	...
	uint16_t *pConfidence = saveIntensity ? (uint16_t*)parts[2].pData : NULL;
	const size_t nPixel = parts[0].width * parts[0].height;

	o.write((char*)pPoint, sizeof(float)*nPixel*3);
...
int main(int argc, char* argv[])
{
	...
	const char* fileName = argc > 1 ? argv[1] : "points3.pcd";
